...
def test_workflow_emits_build_content_inventory_after_compile(monkeypatch):
    ...
    result = asyncio.run(wf.run(request))
    assert result.final_status == FinalStatus.COMPLETED
    triples = _collect_step_lifecycle_calls(captured)
    started = ("BUILD_CONTENT_INVENTORY", "build_content_inventory", "started")
    completed = ("BUILD_CONTENT_INVENTORY", "build_content_inventory", "completed")
    assert started in triples
    assert completed in triples
...
def test_workflow_emits_generate_knowledge_chunks_after_compile(monkeypatch):
    ...
    triples = _collect_step_lifecycle_calls(captured)
    started = (
        "GENERATE_KNOWLEDGE_CHUNKS", "generate_knowledge_chunks", "started",
    )
    completed = (
        "GENERATE_KNOWLEDGE_CHUNKS", "generate_knowledge_chunks", "completed",
    )
    assert started in triples
    assert completed in triples
...
def test_synthetic_steps_fire_in_user_facing_order(monkeypatch):
    ...
    def first_index(predicate):
        return next(
            (i for i, t in enumerate(triples) if predicate(t)), None,
        )

    bci_started = first_index(
        lambda t: t == ("BUILD_CONTENT_INVENTORY", "build_content_inventory", "started"),
    )
    gkc_started = first_index(
        lambda t: t == ("GENERATE_KNOWLEDGE_CHUNKS", "generate_knowledge_chunks", "started"),
    )
    assert bci_started is not None
    assert gkc_started is not None
    assert bci_started < gkc_started
