"""documentation guards + small cleanup.

Pins:
 1. –11 docs exist + describe the new architecture (not the
 legacy one).
 2. Docs do not describe split-mode / pre-compile gating as active
 architecture.
 3. `image_summaries[].warnings` carries per-image errors lifted
 from `metadata.error` ( small cleanup).
"""
...
def test_image_summary_warnings_carry_per_image_error():
    """The cleanup: when `PerImageVisionAdapter` records
