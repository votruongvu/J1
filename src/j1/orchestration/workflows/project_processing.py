...
class ProjectProcessingWorkflow:
    ...
    async def _validate(self, request: ProjectProcessingRequest) -> None:
        self._current_operation = OPERATION_VALIDATE
    ...
    async def _list_documents(
        ...
        self._current_operation = OPERATION_LIST_DOCUMENTS
    ...
    async def _process_document(
        ...
        self._current_operation = f"{OPERATION_COMPILE}:{document_id}"
        ...
        if request.enricher_kind:
            for artifact_id in list(compile_result.artifact_ids):
                ...
                if self._cancelled:
                    return
                self._current_operation = f"{OPERATION_ENRICH}:{artifact_id}"
        ...
        if request.graph_builder_kind:
            ...
            if self._cancelled:
                return
            self._current_operation = OPERATION_BUILD_GRAPH
    ...
    async def _index_all(self, request: ProjectProcessingRequest) -> None:
        self._current_operation = OPERATION_INDEX
    ...
    async def _finalize(self, request: ProjectProcessingRequest) -> None:
        self._current_operation = OPERATION_FINALIZE
        await workflow.execute_activity_method(
