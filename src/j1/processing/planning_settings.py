...
WHAT the pipeline intends to do for a document — modes / steps /
estimated cost / risk-level — plus a lightweight content digest
derived from the parsed-content manifest.

This module owns only the env-var loader for that surface. The
projector itself lives in ``j1.ingestion_review.service``.
...
(``MAX_SAMPLE_BLOCKS``, ``MAX_PREVIEW_CHARS``) are the privacy
boundary.
"""
...
class PlanningSettings:
    ...
    ``llm_planning_enabled`` enables the optional LLM-assisted
    planning pass. Default OFF — rule-based planning is the
    documented baseline.

    ``model_profile`` names the registered FAST/PREMIUM LLM role to
    use when ``llm_planning_enabled=True``. Free-form string so
    deployments can register their own profile names.

    ``max_sample_blocks`` and ``max_preview_chars`` cap the digest
    fed to the LLM planner. Both are privacy boundaries: the
    planner must NEVER see the full raw document.
    """
...
def load_planning_settings(
    ...
    ``ConfigError`` so misconfiguration surfaces at startup rather
    than silently degrading at runtime.
    """
