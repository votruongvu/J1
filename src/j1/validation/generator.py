...
# abstains, which doesn't require a domain-specific question.
# 4+ may layer in an LLM-driven domain-aware variant; for 
...
class DefaultTestCaseGenerator:
 ...
 modalities ship in fixed order: smoke → negatives →
 ...
 """
        ...
        # deterministic negatives drawn from a fixed pool.
        ...
        #  modality cases. Each modality's count is bounded
        ...
        return ValidationSetDTO(
            ...
            metadata={
                ...
                #  surfaces — let the FE/REST layer answer
...
def _negative_case(question: str) -> ValidationTestCaseDTO:
    """an off-topic test case. The runner expects the
