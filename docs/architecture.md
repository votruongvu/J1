...
```
┌─────────────────────────────────────────────────────────┐
│  Protocol adapters    j1.api (REST/FastAPI)             │
...
└──────────────────────────┬──────────────────────────────┘
                           ...
                           ▼
┌─────────────────────────────────────────────────────────┐
│  Application services  ServiceContainer wiring          │
...
1. **Ports** (`j1/integration/ports.py`) — `Protocol` classes that describe what J1 exposes to the outside world. One per capability:
   - `DocumentIngestionPort`, `JobStatusPort`, `SearchPort`, `RetrievalPort`,
     `AnswerPort`, `CitationLookupPort`, `SourceLookupPort`, `FeedbackPort`,
     `EventPublisherPort`
...
- HTTP routing, middleware, or status codes (those belong to `j1.api`)
...
`tests/test_integration_layer.py` includes a static dependency-direction guard that AST-walks every core module and asserts no `import j1.api` or `import j1.integration`. The guard fails the build if anyone reverses the dependency arrow.

The same test file verifies `j1.integration` itself never imports from `j1.api` — adapters depend on integration, not the reverse.

## How to add a future protocol adapter

For a new protocol (e.g. MCP, Webhook, AsyncAPI), create a sibling package:
...
2. Maps it to a DTO (`AnswerRequestDTO`, `FeedbackDTO`, etc.)
...
The existing `j1.api` (FastAPI/REST) is the canonical example. It can be migrated to consume `ApplicationFacade` when the larger refactor is wanted; for now it directly uses `ServiceContainer` (the older wiring path) and works fine. Either is acceptable as long as the dependency direction stays clean.

## Why the dependency direction stays clean

- Core modules (`j1.intake`, `j1.processing`, etc.) never reference `j1.integration` or `j1.api`. The integration layer wraps them; they don't know it exists. This is enforced by `test_core_modules_do_not_import_external_layer`.
- `j1.integration` imports from core to wire ports, but never from `j1.api`. Enforced by `test_integration_does_not_import_protocol_adapters`.
- `j1.api` imports from core and (in future) from `j1.integration`. Free to do so — it's the outermost layer.
