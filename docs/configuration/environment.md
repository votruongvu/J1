...
| Name | Required | Default | Used by | Description |
|---|---|---|---|---|
| `J1_INGEST_PLANNER_ENABLED` | No | `true` | Workflow request defaults | When `true`, each document is profiled pre-compile (`profile_document` activity) and the workflow builds an `initial_execution_plan` carrying the resolved domain pack, enrichment policy, and candidate enrichment modules. Caller-supplied `compilerKind` / `enricherKind` / `graphBuilderKind` / `indexerKind` always override the plan's recommendations. Read by the dev API entrypoint at `deploy/dev/api.py`; production deployments wire the flag through their own bootstrap. Set to `false` to skip profiling and run with caller-supplied kinds only. |
...
| `J1_TEMPORAL_SEARCH_ATTRIBUTES_ENABLED` | No | `false` | Workflow request defaults | When `true`, the workflow upserts `J1IngestStage` + `J1FinalStatus` + retry-count attributes so operators can filter workflows by stage / final status / cost in the Temporal UI. **Pre-register the attributes first** (see [`docs/operations/temporal.md`](../operations/temporal.md) § 5.1) — the Temporal cluster rejects upserts for unregistered attributes at activation completion, and the worker can't recover from it. The dev compose stack auto-registers them via the `temporal-init` service. |
