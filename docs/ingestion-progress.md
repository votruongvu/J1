...
- An **`initial_execution_plan`** + **`post_compile_enrichment_plan`**
  that together document what the system decided to do (run, skip,
  conditional) and *why* — see
  [`architecture/ingestion-pipeline.md`](architecture/ingestion-pipeline.md).
...
> [`architecture/ingestion-pipeline.md`](architecture/ingestion-pipeline.md)
> for the current stage decomposition and
> [`architecture/final-ingestion-report.md`](architecture/final-ingestion-report.md)
> for the run-detail data model. The legacy single-blob `IngestPlan`
> object described in earlier revisions of this section has been
> removed; the current pipeline splits planning into two typed
> artifacts:
>
> * **`initial_execution_plan`** — cheap pre-compile plan
>   (domain pack, enrichment policy, candidate modules).
> * **`post_compile_enrichment_plan`** — produced post-compile;
>   decides whether enrichment runs based on compile evidence.
>
> Both artifacts surface on the run-detail page and feed the
> SSE event timeline described below.
