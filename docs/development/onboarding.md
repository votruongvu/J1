...
```bash.venv/bin/pytest # full suite.venv/bin/pytest -q # quiet mode.venv/bin/pytest -x -v tests/test_intake.py # one file, verbose, fail-fast
```
...
```bash.venv/bin/python -m deploy.dev.api
